...
class EpletAnalysis:
    """Eplet analysis using the EpRegistry API and calibration comparison.

    Can be used in two ways:

    1. **API queries** – build truth and imputed eplet tables from existing
       donor-recipient pair DataFrames::

        ea = EpletAnalysis.from_key_file('api.key')
        truth_eplets = ea.build_truth_eplet_table(
            truth_pairs_df, which_impute='DRDQ',
            save_path='DRDQ_eplet_truth_table.csv')
        impute_eplets = ea.build_impute_eplet_table(
            impute_pairs_df, which_impute='DRDQ',
            save_path='DRDQ_eplet_lowres_impute.csv')

    2. **Calibration analysis** – compare predicted vs true eplet counts::
    ...
    impute_file must have a PairProb_{locus} column (output of build_impute_eplet_table).
    """

    def __init__(self, truth_file: str = None, impute_file: str = None,
                 api_key: str = None, delay: float = 1.0):
        """
        Parameters
        ----------
        truth_file : str, optional
            CSV of truth eplet counts. Required for run_calibration_analysis.
        impute_file : str, optional
            CSV of imputed eplet counts. Required for run_calibration_analysis.
        api_key : str, optional
            EpRegistry API key. Required for build_truth/impute_eplet_table.
        delay : float
            Seconds to sleep between API calls (default 1.0).
        """
        self.api_key = api_key
        ...
        self._prob_cols: Dict[str, str] = {}
        self.pair_results: Dict[str, pd.DataFrame] = {}
        self.summary_results: Dict[str, Dict] = {}

        if truth_file is not None and impute_file is not None:
            self._load_files(truth_file, impute_file)

    @classmethod
    def from_key_file(cls, key_file: str, delay: float = 1.0) -> 'EpletAnalysis':
        """Instantiate by reading the API key from a plain-text file."""
        with open(key_file) as f:
            api_key = f.readline().strip()
        return cls(api_key=api_key, delay=delay)

    def _load_files(self, truth_file: str, impute_file: str):
        ...
        if not self._prob_cols:
            raise ValueError(
                ...
                "Use the output of EpletAnalysis.build_impute_eplet_table()."
            )

    # ------------------------------------------------------------------
    # API: build eplet tables
    # ------------------------------------------------------------------
    ...
    def build_impute_eplet_table(self, impute_pairs_df: pd.DataFrame,
        ...
        ----------
        impute_pairs_df : pd.DataFrame
            Must have columns: DON_ID, REC_ID, PairProb_{which_impute},
            DON_{which_impute}, REC_{which_impute}.
        which_impute : str
            One of 'DRDQ', 'DR', 'DQ'.
        save_path : str, optional
    ...
    def run_calibration_analysis(self, n_bins: int = 10) -> Dict:
        ...
        """
        if self.truth_data is None or self.impute_data is None:
            raise ValueError(
                "truth_file and impute_file must be provided to run calibration analysis. "
                "Pass them to __init__ or call _load_files() first."
            )
    ...
    def _parse_api_response(self, api_result: Dict, id_pair: str) -> Dict:
        """Parse one API response into a flat dict."""
        row: Dict = {'ID': id_pair}
        for key, value in api_result.items():
            if key in _SKIP_API_KEYS:
                continue
